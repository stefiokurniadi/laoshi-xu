...
import type { QuestionMode, WordGameApiPayload } from "@/lib/types";
import {
...
/**
 * Shared implementation for GET /api/word and SSR first card (e.g. Flashcard 2.0).
 */
export async function getWordGamePayload({
  supabase,
  user,
  requestMode,
  pointsSource,
  ...
  user: User | null;
  requestMode: QuestionMode | null;
  pointsSource: "flashcard" | "quiz";
}): Promise<WordGameApiPayload> {
  let maxLevel = 2;
  ...
  const overlapHanzi =
    requestMode === "HZ_TO_EN" || requestMode === "PY_TO_MIX" ? word.hanzi : undefined;
  ...
  return { word, distractors, source: reviewWord ? "review" : "hsk" };
}
